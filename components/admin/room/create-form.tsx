...
const CreateForm = ({amenities}: {amenities: Amenities}) => {
    ...
    const [pending, startTransition] = userTransition();
    ...
    const deleteImage = (image: string) =>{
        startTransition(async()=>{
            try {
                await fetch(`/api/upload/?iamgeUrl=${image}`,{
                    method:"DELETE"
                });
                setImage(" ")
            } catch (error) {
    ...
    <form action={formAction}>
        <div className="grid md:grid-cols-12 gap-5">
            ...
            <div className="col-span-4 bg-white p-4">
                <label htmlFor="input-file" className="flex flex-col mb-4 items-center
                justify-center aspect-video border-2 border-gray-300 border-dashed rounded-md
                cursor-pointer bg-gray-50 relative">
                    <div className="flex flexchea-col items-center justify-center text-gray-500 pt-5 pb-6 z-10">
                ...
                <button type="submit" className={clsx("bg-orange-400 text-white w-full hover:bg-orange-500 py-2.5 px-6 md:px-1 text-lg font-semibold cursor-pointer", {"opacity-50 cursor-progress": isPending,})} disabled={isPending}>
